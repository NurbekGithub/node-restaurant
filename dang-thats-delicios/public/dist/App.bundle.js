...
function typeAhead(search) {
  ...
  searchInput.on('input', function () {
    ...
    _axios2.default.get('/api/search?q=' + this.value).then(function (res) {
      if (res.data.length) {
        searchResult.innerHTML = (0, _dompurify.sanitize)(searchResultHTML(res.data));
        return;
      }
      // tell them that nothing came back;
      searchResult.innerHTML = (0, _dompurify.sanitize)('<div class="search__result">No result for ' + _this.value + ' found!</div>');
    }).catch(function (err) {
      console.error(err);
    });
  });
